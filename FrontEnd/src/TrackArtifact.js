...
export default class TrackArtifact extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    let count = await contract.methods.selfProfile().call({ from: accounts[0] });
    console.log('This is count selleable');
    console.log(count._ownings);
    let responses = [];
    let lngth = count._ownings.length;
    for(let i=0;i<lngth;i++){
      let response = await contract.methods.getProduct(count._ownings[i]).call({from: accounts[0]});
